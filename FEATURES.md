...
- per-call tool output capped at roughly one fifth of the available model context with an explicit truncation marker, and live `shell` output streaming bounded by the same cap, sliced at a character boundary so one delta never overshoots it; the cap is measured after reserving the Tool message's own framing (role, call id, per-message overhead) and floored at 32 tokens so control fields like a job_id survive near the context limit, so a tool result never pushes the next model request past the limit; when even the floor no longer fits, the conversation is compacted mid-turn — keeping the user message, the assistant calls, and the results delivered so far — before the tool runs, and the turn fails with a clear error if compaction cannot free the room
