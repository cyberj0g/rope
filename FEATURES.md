...
- model-managed `update_plan` state persisted across restarts, with visible tool calls and model context carrying only the latest plan — the most recent `update_plan` call (arguments and result) plus the persisted plan stay un-compacted so the model always has a complete in-context example of the call shape, while every older call collapses to a `{"stored": true}` marker, and a call arriving without a `plan` array (e.g. a model imitating that marker) gets a clear error telling it to send the complete plan instead of an opaque decode failure
