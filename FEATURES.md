...
- per-call tool output capped at roughly one fifth of the available model context with an explicit truncation marker, and live `shell` output streaming bounded by the same cap, sliced at a character boundary so one delta never overshoots it; the cap is floored at 32 tokens so control fields like a job_id survive near the context limit, clamped to the remaining context so a tool result never pushes the next model request past the limit
